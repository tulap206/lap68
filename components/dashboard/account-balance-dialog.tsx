...
export function AccountBalanceDialog({
  ...
  const handleSave = async () => {
    ...
    try {
      await savePortfolioSettings(user.id, {
        ...
        primary_account_id:
          settings.primary_account_id || accounts[0]?.id || null,
      });
      logAction(
        "Cập nhật số dư TK",
        `${accounts.length} tài khoản · ${displayMoney(totalLiquidBalance({ liquid_accounts: accounts, updated_at: null }))}`,
      );
  ...
  const previewTotal = rows.reduce(
    (sum, r) => sum + parseMoneyInput(r.balance),
    0,
  );
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
            ...
            >
              ...
              <div className="space-y-1">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    updateRow(row.id, {
                      balance: formatMoneyInput(e.target.value),
                    })
                  }
                  placeholder="0"
                />
        ...
        <div className="rounded-lg bg-muted/50 border border-border px-3 py-2 flex items-center justify-between">
          ...
          <span className="font-mono font-bold text-[#1f6c9f] tabular-nums">
            {displayMoney(previewTotal)}
          </span>
        </div>
