...
import {
  adjustBusinessCapital,
  setBusinessOpeningCapital,
} from "@/lib/supabase";
...
export function CapitalAdjustDialog({
    ...
    if (open) {
      ...
      if (selected) {
        setOpeningAmount(
          formatMoneyInput(
            String(
              parseBusinessCapital(selected.ghi_chu).opening_capital || "",
            ),
          ),
        );
      }
  ...
  useEffect(() => {
    ...
    setOpeningAmount(
      formatMoneyInput(
        String(parseBusinessCapital(selected.ghi_chu).opening_capital || ""),
      ),
    );
  }, [businessId, selected]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (mode === "opening") {
        ...
        logAction(
          capitalAdjustLabel(adjustType),
          `${selected.name}: ${displayMoney(parsed)}`,
        );
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        ...
        <DialogHeader>
          ...
          <DialogDescription>
            Nạp/rút vốn chủ hoặc đặt vốn ban đầu — không tính vào thu/chi kinh
            doanh
          </DialogDescription>
        </DialogHeader>
          ...
          {mode === "adjust" ? (
            <>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="space-y-2">
                  ...
                  <Select
                    value={adjustType}
                    onValueChange={(v) =>
                      setAdjustType(v as "deposit" | "withdraw")
                    }
                  >
                    <SelectTrigger>
                      <SelectValue />
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="VD: Bổ sung quỹ mặt bằng"
                />
              </div>
            </>
          ) : (
            <div className="space-y-2">
              ...
              <Input
                ...
                value={openingAmount}
                onChange={(e) =>
                  setOpeningAmount(formatMoneyInput(e.target.value))
                }
              />
              <p className="text-[11px] text-muted-foreground">
                Vốn gốc trước khi ghi nhận thu chi. Hiện tại:{" "}
                <span className="text-muted-foreground font-mono">
                  {displayMoney(meta?.opening_capital || 0)}
                </span>
              </p>
            </div>
          )}
