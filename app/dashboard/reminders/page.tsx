  ...
  return (
    <ModulePageShell module="cashflow">
      <ModuleSubpageHeader
        module="cashflow"
        title="Nhắc hẹn"
        subtitle="Theo dõi thu/chi sắp đến hạn trên tất cả việc kinh doanh"
      />
                ...
                {grouped.overdue.map((r) => (
                  <div key={`${r.schedule.id}-${r.dueDate}`}>
                    <p className="text-xs text-muted-foreground mb-1">
                      {businessMap.get(r.schedule.business_id) || "—"}
                    </p>
            ...
            <div className="p-4 space-y-2">
              {grouped.soon.length === 0 ? (
                <p className="text-sm text-muted-foreground text-center py-4">
                  Không có nhắc hẹn trong khoảng này
                </p>
              ) : (
                grouped.soon.map((r) => (
                  <div key={`${r.schedule.id}-${r.dueDate}`}>
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              Đã {completeTarget?.direction === "collect" ? "thu" : "chi"} —{" "}
