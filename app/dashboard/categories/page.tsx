...
import { fetchCategories, insertCategory, updateCategory, deleteCategory, type Category } from "@/lib/supabase"
...
export default function CategoriesPage() {
  ...
  const loadData = useCallback(async () => {
    if (!user) return
    try {
      setCategories(await fetchCategories(user.id))
    } catch {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editing) {
        ...
        await insertCategory({ user_id: user.id, name: form.name, type: form.type, color: form.color, icon: "wallet" })
  ...
  const handleDelete = async (c: Category) => {
    ...
    try {
      ...
      toast.error("Không thể xóa — có thể đang được dùng trong giao dịch")
    }
  }
  ...
  return (
    <ModulePageShell module="cashflow">
      <ModuleSubpageHeader
        ...
        subtitle="Phân loại giao dịch theo nghiệp vụ"
        actions={<AccentButton module="cashflow" onClick={openCreate}><Plus className="h-4 w-4" /> Thêm danh mục</AccentButton>}
      />
          ...
          <ModuleResponsiveTable
            ...
            rows={categories.map((c) => [
              ...
              <span key="color" className="inline-flex items-center gap-2">
                <span className="h-4 w-4 rounded-full border" style={{ backgroundColor: c.color }} />
                {c.color}
              </span>,
